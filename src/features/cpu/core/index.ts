...
import { InputSignals, OutputSignals, Signals, MAX_PORT } from '@/features/io/core'
...
type StepArgs = [memoryData: MemoryData, cpuRegisters: Registers, inputSignals: InputSignals]

export type StepResult = ExcludeTupleTail<StepArgs>

export const step = (...args: StepArgs): [...StepResult, Signals] => {
  const [__memoryData, __cpuRegisters, __inputSignals] = args
  const __outputSignals: OutputSignals = {}

  const getInput = (): InputSignals => __inputSignals
  ...
  const stepResult: StepResult = createNextState(
    [__memoryData, __cpuRegisters],
    ([memoryData, cpuRegisters]) => {
