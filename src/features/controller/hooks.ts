...
const step = async (): Promise<void> => {
  ...
  lastStep = new Promise(resolve => {
    ...
    if (breakpoints.length > 0 && hasStatement && isRunning && !willSuspend) {
      ...
      const willBreak = breakpoints.some(lineRange => lineRangesOverlap(lineRange, statementRange))
      if (willBreak) {
...
export const useController = (): Controller => {
  ...
  useEffect(() => {
    return listenAction(setAssemblerState, fullyStop)
  }, [])
